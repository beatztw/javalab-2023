...
func doRequestToRemoteServer(result chan<- string) {
...
func goToRemoteServer(ctx context.Context) (result string, err error) {
	...
	go doRequestToRemoteServer(resultChannel)

	for {
		select {
		case <-ctxWithTimeout.Done():
			if errors.Is(ctxWithTimeout.Err(), context.DeadlineExceeded) {
				fmt.Println("Timeout... RemoteServer is cancelled.")
				return "", ctxWithTimeout.Err()
			}
			if errors.Is(ctx.Err(), context.Canceled) {
				fmt.Println("RemoteServer is cancelled.")
				return "", nil
			}
